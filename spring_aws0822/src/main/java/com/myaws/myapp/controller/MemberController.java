...
public class MemberController {		// 컨트롤러 용도의 객체를 생성해달라고 스프링에 요청하기  > @Controller > import
	...
	public JSONObject JSONObject (@RequestParam("memberId") String memberId) {  //String memberId = request.getParameter("memberid"); >> @RequestParam("memberId") String memberId		
...
//그전 dao에 메서드 만들었던것 처럼 여기서는 service로 가서 메서드 선언 인터페이스 생성 구현 > 그다음 마이바티스 메서드 가져오고 매퍼로 가서  쿼리 작성해준다
	...
	public String memberLoginAction(
			...
			RedirectAttributes rttr		// 한번만 담아서 사용하는 클래스 생성 > 클래스명 지정
			// 모델의 기능도 가지고 있다
